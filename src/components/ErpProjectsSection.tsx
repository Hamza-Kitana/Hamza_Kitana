...
export const ErpProjectCard = ({ project, index, onClick, className = '' }: ErpProjectCardProps) => {
  ...
  return (
    <motion.button
      ...
      whileHover={{ y: -6 }}
      className={`group relative w-full aspect-[3/4] sm:aspect-[4/5] text-start cursor-pointer rounded-xl overflow-hidden border border-primary/25 hover:border-primary/50 transition-all duration-500 hover:shadow-[0_0_30px_hsl(var(--primary)/0.2)] active:scale-[0.98] ${className}`}
    >
      ...
      <div className="absolute inset-x-0 bottom-0 z-10 p-3 sm:p-4 md:p-5 pt-12 sm:pt-16">
        <h4 className="text-xs sm:text-sm md:text-base font-bold text-white leading-snug line-clamp-3 drop-shadow-[0_2px_8px_rgba(0,0,0,0.85)] group-hover:text-primary transition-colors duration-300">
          {title}
        </h4>
      </div>
    </motion.button>
  ...
  return (
    ...
    >
      ...
      <div className="relative rounded-2xl sm:rounded-3xl border border-primary/25 bg-gradient-to-br from-card/80 via-background/90 to-card/70 backdrop-blur-xl overflow-hidden shadow-[0_0_60px_hsl(var(--primary)/0.08)]">
        ...
        <div className="px-3 py-5 sm:px-5 sm:py-8 md:px-10 md:py-10 space-y-5 sm:space-y-8">
          ...
          <div className="sm:hidden -mx-3 px-3">
            <div className="flex gap-3 overflow-x-auto snap-x snap-mandatory pb-1 scrollbar-hide overscroll-x-contain">
              {projects.map((project, index) => (
                <div
                  key={project.id}
                  className="snap-start shrink-0 w-[38vw] min-w-[128px] max-w-[150px]"
                >
                  <ErpProjectCard
                    project={project}
                    index={index}
          ...
          {/* Tablet & desktop: grid */}
          <div className="hidden sm:grid sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 md:gap-5">
            {projects.map((project, index) => (
              <ErpProjectCard
                key={project.id}
                project={project}
